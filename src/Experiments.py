...
model_linear_regression_wine1 = src.logistic_regression.Logistic(wines.shape[1], 1000, 0.004, lambda x, y: x)
model_linear_regression_wine2 = src.logistic_regression.Logistic(wines.shape[1], 1000, 0.004, lambda x, y: x/np.round(np.log10(y),0) if y > 10 else x)
...
start_time = time.time()
acc = src.CrossValidation.CrossValidation(wines.copy(), model_linear_regression_wine1, 5)
print(time.time() - start_time)
print(acc)
